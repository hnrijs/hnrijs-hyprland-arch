...
body=''
for name in ['caffeine','download','media','exif','ip','speed','disks','clipboard','weather','color']:
...
(target/'Task.qml').write_text('import QtQuick\nQtObject { property bool running:false; property var result:({}); property var progress:({}); property string error:""; signal finished(bool success); function start(a){} }')
...
for font in (base/'preview-fonts').glob('*.ttf'): QFontDatabase.addApplicationFont(str(font))
...
for page,tool in [('clipboard',''),('wallpaper',''),('auth',''),('calendar',''),('control',''),('menu',''),('tools',''),('settings',''),('power',''),('color',''),('web',''),('emoji',''),('tool','download'),('tool','speed'),('tool','periodic'),('tool','media'),('tool','exif'),('tool','defaults')]:
...
def visual_find(item,name):
 ...
 return None
for tool,expected in [('download','mp3'),('media','silent')]:
