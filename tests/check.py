...
class Checks(unittest.TestCase):
    ...
    def test_removed_features(self):
        ...
        for forbidden in ['firefox','--personal','chmod 666','/usr/bin/xterm']:
    ...
    def test_power_cycle_debounces_duplicates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual((d/'changes').read_text().splitlines(),['performance'])
    ...
    def test_v6_packages_and_binds(self):
        install=(ROOT/'install.sh').read_text()
        packages=re.search(r'required=\((.*?)\)',install,re.S)[1].split()
        self.assertEqual(len(packages),len(set(packages)))
        for name in ['pavucontrol','base-devel','libreoffice-fresh','libqalculate','ufw']:
            self.assertIn(name,packages)
        self.assertNotIn('librewolf',packages)
        lua=(ROOT/'config/hypr/hyprland.lua').read_text()
        for key in ['SHIFT + G','SHIFT + H','SHIFT + W','SHIFT + N','SHIFT + I','SHIFT + P']:
            self.assertNotIn('bind("'+key+'"',lua)
        self.assertNotIn('xdg-terminal-exec',lua)
        self.assertEqual(install.count('yay -S --needed helium-browser-bin'),1)

    def test_network_changes_only_dns(self):
        net=module('networksettings',ROOT/'config/scripts/network-settings.py')
        row={'uuid':'12345678-1234-1234-1234-123456789abc','device':'wlan0','name':'Home'}
        with patch.object(net,'profiles',return_value=[row]), patch.object(net,'run',return_value='') as run, patch.object(net.subprocess,'run') as apply:
            apply.return_value.returncode=1
            result=net.main('dns',row['uuid'],'dhcp')
            command=run.call_args.args[0]
            self.assertEqual(command[:5],['nmcli','connection','modify','uuid',row['uuid']])
            self.assertIn('ipv6.ignore-auto-dns',command)
            self.assertNotIn('ipv4.method',command)
            self.assertIn('Reconnect',result['message'])
            with self.assertRaises(ValueError):net.main('dns',row['uuid'],'invalid')

    def test_notification_preferences_boundaries(self):
        task=module('notifprefs',ROOT/'config/scripts/hshell-task.py')
        with tempfile.TemporaryDirectory() as tmp:
            task.STATE=Path(tmp);task.PREFS=Path(tmp)/'preferences.json'
            for value in ['0.1','3','10']: self.assertEqual(task.save_pref('notificationSeconds',value)['notificationSeconds'],value)
            for value in ['0','10.1','nan']:
                with self.assertRaises(ValueError):task.save_pref('notificationSeconds',value)
            with self.assertRaises(ValueError):task.save_pref('accentColor','red;command')
