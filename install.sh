...
for arg in "$@"; do
    case $arg in
        ...
        --help) printf 'Run as your normal user: bash install.sh [--config-only] [--no-lightdm]\n'; exit 0 ;;
...
if ! $config_only; then
    ...
    required=(
        ...
        pipewire pipewire-alsa pipewire-pulse playerctl polkit power-profiles-daemon python
