...
def save_pref(key, value):
    if key not in BOOL_PREFS + ['city']: raise ValueError('Unknown preference')
...
def default_apps(action, role='', desktop=''):
    import configparser
    config=Path(os.environ.get('XDG_CONFIG_HOME',str(Path.home()/'.config')))
    data=Path(os.environ.get('XDG_DATA_HOME',str(Path.home()/'.local/share')))
    folders=[data/'applications']+[Path(p)/'applications' for p in os.environ.get('XDG_DATA_DIRS','/usr/local/share:/usr/share').split(':')]
    entries={}
    for folder in folders:
        if not folder.is_dir(): continue
        for file in sorted(folder.rglob('*.desktop')):
            ident=str(file.relative_to(folder)).replace('/','-')
            if ident in entries: continue
            parser=configparser.ConfigParser(interpolation=None,strict=False)
            try:
                parser.read(file,encoding='utf-8'); d=parser['Desktop Entry']
                if d.get('Type')!='Application' or d.get('Hidden','false')=='true': continue
                entries[ident]={'id':ident,'name':d.get('Name',ident),'terminal':'TerminalEmulator' in d.get('Categories','').split(';')}
            except (ValueError,KeyError,configparser.Error): continue
    path=config/'applications/defaults.json'
    defaults=json.loads(path.read_text())
    if action=='set':
        if role not in ['browser','video','image','files','text','terminal'] or desktop not in entries: raise ValueError('Invalid application selection')
        if role=='terminal' and not entries[desktop]['terminal']: raise ValueError('Choose a terminal emulator')
        previous=path.read_text(); defaults[role]=desktop
        tmp=path.with_suffix('.tmp');tmp.write_text(json.dumps(defaults,indent=2));tmp.replace(path)
        try: subprocess.run([sys.executable,str(SCRIPTS/'default-apps.py')],check=True,capture_output=True,text=True)
        except Exception:
            path.write_text(previous);raise
    elif action!='list': raise ValueError('Unknown default-apps action')
    return {'apps':sorted(entries.values(),key=lambda a:a['name'].lower()),'defaults':defaults}
...
def main(action, *args):
    ...
    if action=='default-apps':return default_apps(*args)
    if action=='power-profiles':
        output=subprocess.check_output(['powerprofilesctl','list'],text=True)
        return {'profiles':list(dict.fromkeys(re.findall(r'^\s*\*?\s*(power-saver|balanced|performance):',output,re.M)))}
