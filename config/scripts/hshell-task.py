...
THEMES = ['mocha', 'monochrome', 'tokyo', 'gruvbox', 'nord']
...
def save_pref(key, value):
    if key == 'theme' and value not in THEMES: raise ValueError('Unknown theme')
    if key not in ['theme','city','syncLogin']: raise ValueError('Unknown preference')
