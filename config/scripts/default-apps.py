...
def main():
    ...
    terminal=defaults.get('terminal','Alacritty.desktop')
    if not isinstance(terminal,str) or not terminal.endswith('.desktop') or '/' in terminal or '\n' in terminal: raise ValueError('Invalid terminal entry')
    (CONFIG/'xdg-terminals.list').write_text(terminal+'\n')
    (CONFIG/'hyprland-xdg-terminals.list').write_text(terminal+'\n')
    subprocess.run(['update-desktop-database',str(apps)],check=True)
    print('Default apps updated. Neovim uses the selected terminal.')
