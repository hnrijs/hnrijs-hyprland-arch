...
FocusScope {
    ...
    Rectangle {
        anchors.fill: parent
        radius: height / 2
        color: root.filled ? Style.bg2 : Style.bg0
        border.width: root.filled ? (root.activeFocus ? 2 : 0) : 1
        border.color: root.activeFocus ? Style.primary : Style.bg2
    ...
    ShellText {
        ...
        text: root.icon
        color: root.filled ? Style.bgDim : (root.enabled ? Style.muted : Style.mutedDark)
        font.pixelSize: 14
    ...
    ShellText {
        ...
        text: root.valueText
        color: root.filled && filledTrack.x + filledBar.width >= x + width / 2 ? Style.bgDim : Style.foreground
        font.pixelSize: 10
        font.weight: Font.Bold
    }
