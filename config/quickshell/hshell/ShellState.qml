...
Singleton {
    ...
    function setPanel(name) {
        if (!["clock", "control", "launcher", "clipboard", "calendar", "wallpaper", "power", "menu", "auth", "tool", "detail", "tools", "settings", "emoji", "color", "web"].includes(name))
            return;
        if (!expanded)
    ...
    Connections {
        ...
        function onFocusedWorkspaceChanged() {
            if (root.panel !== "auth")
                root.close();
            root.osd = "";
            root.workspaceNotice = Preferences.workspaceOsd;
