...
Item {
    ...
    function checkBattery() {
        ...
        if (level < 0.10 && !alerted) {
            alerted = true;
            lowBatteryNotification.command = ["gdbus", "call", "--session", "--dest", "org.freedesktop.Notifications", "--object-path", "/org/freedesktop/Notifications", "--method", "org.freedesktop.Notifications.Notify", "Power", "0", "battery-caution", "Battery level is critical", Math.round(level * 100) + "% remaining · connect a charger soon.", "[]", "{'urgency': <byte 2>}", "0"];
